...
for gaborFilter in gaborFilterBank:
    ...
    #img = img.astype(float)
    #print("gaborFilter[\"filterPairs\"] Shape {} Type Img {} Img dtype {}".format(gaborFilter["filterPairs"].shape, type(img), img.dtype))
...
for i, fm in enumerate(featureMaps):
    real_part = fm[...,0]
    imag_part = fm[...,1]
    #print("Real part shape {}, Imag part shape {}".format(real_part.shape, imag_part.shape))
    mag = np.sqrt(real_part**2 + imag_part**2)  #None  # \\TODO: Compute the magnitude here
...
kernel_size = 5
sigma = 2
x_range = np.linspace(-1*np.floor(kernel_size/2), np.floor(kernel_size/2), kernel_size)
kernel = np.exp(-1*(x_range**2)/(2*(sigma*sigma)))/(sigma*np.sqrt(2*np.pi))
...
if smoothingFlag:
    for i, fmag in enumerate(featureMags):
        #print(type(fmag), fmag.shape, type(kernel))
...
#print("Features means {}, max {} min {}".format(features.mean(axis=0), features.max(axis=0), features.min(axis=0)))

from sklearn import preprocessing
features =   preprocessing.scale(features) 	# \\ TODO: i)  Implement standardization on matrix called features.
...
plt.axis("off")
plt.savefig(os.path.join(plots_path, 'pca.png'))
plt.show()
...
tic = time.time()
pixLabels = KMeans(n_clusters=k, random_state=0).fit(features).labels_ #None  # \\TODO: Return cluster labels per pixel
print(set(pixLabels))
ctime = time.time() - tic
...
#print("pixLabels shape {}".format(pixLabels.shape))
plt.figure()
plt.title(f'Pixel clusters')
plt.imshow(pixLabels)
plt.axis("off")
plt.savefig(os.path.join(plots_path, 'pixel_clusters.png'))
plt.show()
...
plt.axis("off") 
plt.savefig(os.path.join(plots_path, 'montage.png'))
plt.show()
